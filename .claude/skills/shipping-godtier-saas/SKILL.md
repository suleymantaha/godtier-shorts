...
**REQUIRED BACKGROUND:** Use `superpowers:test-driven-development` for code changes, `superpowers:systematic-debugging` for failures, and `superpowers:verification-before-completion` before claiming completion.

Read `references/production-contract.md` for detailed billing, abuse, queue, storage, GPU and security contracts. If present, also read `docs/superpowers/plans/2026-08-20-godtier-shorts-production-v1.md` before architectural changes.
