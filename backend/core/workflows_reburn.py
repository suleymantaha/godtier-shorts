...
from backend.core.subtitle_timing import canonicalize_transcript_segments
...
class ReburnWorkflow:
    ...
    async def run(
        ...
        from backend.core.workflow_runtime import create_subtitle_renderer, resolve_output_video_path, resolve_subtitle_render_plan
        ...
        async with self.ctx.acquire_gpu_stage(
            wait_message="Reburn için GPU render sırası bekleniyor...",
            active_message="Reburn için GPU render slotu alındı.",
            wait_progress=35,
            active_progress=40,
        ):
        ...
        normalized_transcript = canonicalize_transcript_segments(self.ctx._normalize_transcript_payload(transcript))
        ...
        resolved_project_id = project_id or (existing_render_metadata.get("project_id") if isinstance(existing_render_metadata, dict) else None)
