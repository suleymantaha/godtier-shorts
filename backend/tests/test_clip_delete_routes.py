...
def test_delete_project_short_removes_managed_assets_and_preserves_project_files(
    ...
    response = client.delete("/api/projects/proj_1/shorts/clip_1.mp4", headers=auth_headers["editor"])
    ...
    assert invalidate_calls == ["clip_deleted:proj_1/clip_1.mp4"]
...
def test_delete_project_short_handles_missing_optional_assets(
    ...
    project_root = tmp_path / "projects"
    shorts_dir = project_root / "proj_2" / "shorts"
    shorts_dir.mkdir(parents=True, exist_ok=True)
    ...
    client = TestClient(_build_app())
    response = client.delete("/api/projects/proj_2/shorts/clip_2.mp4", headers=auth_headers["producer"])
...
def test_delete_project_short_returns_not_found_without_invalidation(
    ...
    project_root = tmp_path / "projects"
    shorts_dir = project_root / "proj_3" / "shorts"
    shorts_dir.mkdir(parents=True, exist_ok=True)
    ...
    client = TestClient(_build_app())
    response = client.delete("/api/projects/proj_3/shorts/missing.mp4", headers=auth_headers["editor"])
...
def test_delete_project_short_requires_delete_policy(
    ...
    project_root = tmp_path / "projects"
    shorts_dir = project_root / "proj_4" / "shorts"
    shorts_dir.mkdir(parents=True, exist_ok=True)
    ...
    client = TestClient(_build_app())
    response = client.delete("/api/projects/proj_4/shorts/clip_4.mp4", headers=auth_headers["viewer"])
